                            ...
                            @else
                                <button class="btn btn-primary w-45 me-8 mb-4" data-bs-toggle="modal"
                                    data-bs-target="#modal-tambah" id="tambah"> <i data-feather="plus-circle"
                                        class="w-4 h-4 me-2"></i> Tambah Sender WA </button>
                            @endif
                        </div>
                    </div>
                    <br>
                    <div class="intro-y overflow-auto overflow-lg-visible mt-8 mt-sm-0">
                        <table class="table table-bordered display" id="tabel-device" width="100%" cellspacing="0">
                            <thead>
                                <tr>
                                    <th class="text-nowrap">No Sender WA</th>
        ...
        <div class="modal-dialog">
            <div class="modal-content">
                ...
                <div class="modal-body grid grid-cols-12 gap-4 gap-y-3">
                    <div class="g-col-12">
                        <label for="pos-form-2" class="form-label">No Sender WA</label>
                        <input type="text" class="form-control flex-1" placeholder="08XXXXXXX" id="device">
                    </div>
...
@section('js')
    ...
    <script>
                ...
                success: function(res) {
                    if ($.isEmptyObject(res.error)) {
                        ...
                        if (res.status == 'gagal') {
                            ...
                            location.replace("{{ route('device.index') }}")
                            swal(res.text, {
                                icon: "success",
                            });
        ...
        function edit() {
            let id_edit = $("#id").val()
            let url_edit = "{{ route('device.update', ':id') }}"
